<template>
    <div class="align">
        <div style="padding-top: 10px;"><input placeholder="Project Id (please put the id which you want to update)" type="number" class="inpMen" v-model="data.project_id"/></div>
 ...
 export default {
    data()
    {
        return{
            ...
            data:{project_id:null,projectname:"",description:"",contactprofessional:"",toolsconcerns:"",dsi:"",estload:0,startdate:"",enddate:"",status:false,commentpro:""}
        }
    },
    methods:{
        async SubmitFunc()
        {
            if (this.data.project_id && this.data.projectname && this.data.contactprofessional && this.data.toolsconcerns && this.data.dsi && this.data.startdate && this.data.enddate && this.data.commentpro) {
                axios.post(`${this.$api_url}/UpdateProject`,this.data)
                .then(Response=>{
