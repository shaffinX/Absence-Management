<template>
    <div class="align">
        <div style="padding-top: 10px;"><input placeholder="Absence ID" type="number" class="inpMen" v-model="data.absence_id"/></div>
 ...
 export default {
    ...
    {
        return{
            data:{absence_id:null,typeabscence:"",startdate:"",enddate:"",status:false,userid:Cookies.get('TokkenId')},
        }
    },
    methods:{
        async SubmitFunc()
        {
           if (this.data.absence_id && this.data.typeabscence && this.data.startdate && this.data.enddate) {
                axios.post(`${this.$api_url}/AbscentUpdate`,this.data)
                .then(Response=>{
